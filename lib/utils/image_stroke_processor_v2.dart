...
class ImageStrokeProcessorV2 {
  ...
  static Future<ui.Image> addStrokeToImage(
    ...
    for (int y = 0; y < originalHeight; y++) {
      for (int x = 0; x < originalWidth; x++) {
        ...
        final int paddedIndex = ((y + padding) * paddedWidth + (x + padding)) * 4;
    ...
    final List<List<double>> distanceMap = _computeDistanceTransform(invertedAlpha);
    
    // Apply stroke transformation (change_matrix function from Python)
    final List<List<double>> strokeMask = _changeMatrix(distanceMap, strokeSize);
    ...
    for (int y = 0; y < paddedHeight; y++) {
      for (int x = 0; x < paddedWidth; x++) {
        ...
        if (strokeAlpha > 0) {
          ...
          strokePixels[index + 3] = (strokeAlpha * 255).round().clamp(0, 255); // A
        } else {
    ...
    final Uint8List resultPixels = _alphaComposite(strokePixels, paddedPixels, paddedWidth, paddedHeight);
    
    // Create final image
    return await _createImageFromPixels(resultPixels, paddedWidth, paddedHeight);
  }
  ...
  static List<List<double>> _computeDistanceTransform(List<List<int>> binaryImage) {
  ...
  static List<List<double>> _changeMatrix(List<List<double>> inputMat, int strokeSize) {
